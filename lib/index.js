...
var SearXNGEngine = class {
  ...
  async fetchJson(query, signal) {
    ...
    if (!response.ok) {
      ...
      throw new WebError8(`SearXNG search request failed (HTTP ${response.status})`, "WEB_PROVIDER_ERROR");
    }
    let text;
    try {
    ...
    try {
      ...
      throw new WebError8("SearXNG returned a non-JSON response (is the JSON API enabled?)", "WEB_PROVIDER_ERROR");
    }
  }
  /** Drop blocked domains and duplicate URLs (first occurrence wins). */
  filterAndDedupe(results) {
...
async function enrichSources(query, sources, keep, options, signal) {
  ...
  if (signal.aborted) throw new WebError9("web search aborted", "WEB_ABORTED");
...
async function fetchPageText(url, options, signal) {
  ...
  try {
    ...
    try {
      ...
      if (signal.aborted) throw new WebError9("web search aborted", "WEB_ABORTED", { cause: error });
      return void 0;
    }
    if (!response.ok) {
      await response.body?.cancel();
...
var MultiSearchProvider = class {
  ...
  async fallback(ids, query, maxResults, signal) {
    ...
    throw new WebError10(`all search engines failed: ${errors.join("; ")}`, "WEB_PROVIDER_ERROR");
  }
...
function toWebError(error, message) {
  if (error instanceof WebError10) return error;
  return new WebError10(message, "WEB_ABORTED", { cause: error });
}
function errorMessage(error) {
  if (error instanceof WebError10) return error.message;
  return String(error);
}
...
var Config4 = z4.object({
  ...
  embedding: z4.object({ endpoint: z4.string().default(""), model: z4.string().default("") }).default({})
});
