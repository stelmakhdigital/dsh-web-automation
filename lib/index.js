...
// src/platforms/index.ts
var WEB_PLATFORMS_SETTINGS_NAMESPACE = "web-platforms";
...
function apply3(ctx, config) {
  ...
  ctx.inject(["settings"], (settingsCtx) => {
    settingsCtx.settings.installSection(ctx, WEB_PLATFORMS_SETTINGS_NAMESPACE, Config3, config, {
      setSource: (source) => {
        current = source;
      },
      onChange: () => {
...
import { WebError as WebError13 } from "@deepseek-ai/dsh-web";
...
var ExaEngine = class {
  ...
  async search(query, maxResults, signal) {
    ...
    if (this.provider === void 0 || this.providerKey !== this.cachedKey) {
...
var JinaEngine = class {
  ...
  async search(query, maxResults, signal) {
    ...
    if (!response.ok) {
      await response.body?.cancel();
      throw new WebError9(`Jina search request failed (HTTP ${response.status})`, "WEB_PROVIDER_ERROR");
    }
    let body;
    try {
    ...
    try {
      ...
      throw new WebError9(`Jina search returned a non-JSON response: ${String(error)}`, "WEB_PROVIDER_ERROR", { cause: error });
    }
    const sources = [];
...
var SearXNGEngine = class {
  ...
  async fetchJson(query, signal) {
    ...
    if (!response.ok) {
      ...
      throw new WebError10(`SearXNG search request failed (HTTP ${response.status})`, "WEB_PROVIDER_ERROR");
    }
    let text;
    try {
    ...
    try {
      ...
      throw new WebError10("SearXNG returned a non-JSON response (is the JSON API enabled?)", "WEB_PROVIDER_ERROR");
    }
  }
  /** Drop blocked domains and duplicate URLs (first occurrence wins). */
  filterAndDedupe(results) {
...
async function enrichSources(query, sources, keep, options, signal) {
  ...
  if (signal.aborted) throw new WebError11("web search aborted", "WEB_ABORTED");
...
async function fetchPageText(url, options, signal) {
  ...
  try {
    ...
    try {
      ...
      if (signal.aborted) throw new WebError11("web search aborted", "WEB_ABORTED", { cause: error });
      return void 0;
    }
    if (!response.ok) {
      await response.body?.cancel();
...
var MultiSearchProvider = class {
  ...
  async fallback(ids, query, maxResults, signal) {
    ...
    throw new WebError12(`all search engines failed: ${errors.join("; ")}`, "WEB_PROVIDER_ERROR");
  }
...
function toWebError(error, message) {
  if (error instanceof WebError12) return error;
  return new WebError12(message, "WEB_ABORTED", { cause: error });
}
function errorMessage(error) {
  if (error instanceof WebError12) return error.message;
  return String(error);
}
...
var WEB_SEARCH_MULTI_SETTINGS_NAMESPACE = "web-search-multi";
...
function apply4(ctx, config, options = {}) {
  let current = () => config;
  ctx.inject(["settings"], (settingsCtx) => {
    settingsCtx.settings.installSection(ctx, WEB_SEARCH_MULTI_SETTINGS_NAMESPACE, Config4, config, {
      setSource: (source) => {
        current = source;
      },
      onChange: () => {
  ...
  try {
    ...
    if (error instanceof WebError13 && error.code === "WEB_DUPLICATE_PROVIDER") {
      throw new WebError13(
