...
// src/store/schema.ts
var WEB_STORE_SCHEMA_VERSION = 1;
...
var WebStore = class {
  ...
  async readSearch(cacheKey) {
    ...
    return row === void 0 ? void 0 : mapSearchRow(row);
  }
  ...
  async readPage(normalizedUrl) {
    ...
    return row === void 0 ? void 0 : mapPageRow(row);
  }
...
// src/fetch/index.ts
var DEFAULT_USER_AGENT = "deepseek-harness/0.0.1 (+https://github.com/deepseek-ai)";
var Config = z.object({
...
function apply2(ctx, config) {
  const resolved = config;
  const store = new WebStore({ path: config.storePath ?? dshHomePath2("web.db") });
...
function classifyPlatformError(error, signal, context) {
  ...
  if (timeout !== void 0) return new WebError3("platform search timed out", "WEB_SEARCH_TIMEOUT", { cause: timeout });
  if (signal.aborted) return new WebError3("platform search aborted", "WEB_ABORTED", { cause: error });
  return new WebError3(`${context}: ${error instanceof Error ? error.message : String(error)}`, "WEB_PROVIDER_ERROR", { cause: error });
}
...
async function fetchText(url, headers, signal, maxBytes) {
  let response;
  try {
    const init = { signal, redirect: "follow" };
  ...
  if (!response.ok) {
    throw new WebError3(`platform search got HTTP ${response.status} from ${url}`, "WEB_PROVIDER_ERROR");
  }
  try {
...
function parseByFormat(platform, body, url) {
  switch (platform.format) {
    case "html":
      if (platform.selectors === void 0) throw new WebError3(`platform "${platform.id}" is missing html selectors`, "WEB_PROVIDER_ERROR");
      return parseHtmlResults(body, platform.selectors, url);
    case "json":
      if (platform.fields === void 0) throw new WebError3(`platform "${platform.id}" is missing json fields`, "WEB_PROVIDER_ERROR");
      return parseJsonResults(body, platform.fields);
    case "json-in-html":
      if (platform.jsonInHtml === void 0) throw new WebError3(`platform "${platform.id}" is missing jsonInHtml config`, "WEB_PROVIDER_ERROR");
...
async function searchPlatform(args, deps, signal) {
  ...
  try {
    ...
    if (platform === void 0) {
      ...
      throw new WebError3(`unknown platform "${args.platform}"; available: ${available}`, "WEB_PROVIDER_ERROR");
    }
    ...
    if (platform.format === "rss") {
      ...
      if (platform.searchUrl === void 0) throw new WebError3(`platform "${platform.id}" has no searchUrl`, "WEB_PROVIDER_ERROR");
      url = expandTemplate(platform.searchUrl, { query: args.query, limit });
    }
    if (!URL.canParse(url) || !/^https?:/i.test(url)) {
      throw new WebError3(`platform "${platform.id}" produced an invalid URL: ${url}`, "WEB_INVALID_URL");
    }
...
function classifyWebError(error, signal, context) {
  ...
  if (timeout !== void 0) return new WebError4("web search timed out", "WEB_SEARCH_TIMEOUT", { cause: timeout });
  if (signal.aborted) return new WebError4("web search aborted", "WEB_ABORTED", { cause: error });
  return new WebError4(`${context}: ${String(error)}`, "WEB_PROVIDER_ERROR", { cause: error });
}
...
var BingEngine = class {
  ...
  async fetchSerp(query, maxResults, signal) {
    ...
    if (!response.ok) {
      await response.body?.cancel();
      throw new WebError5(`Bing search request failed (HTTP ${response.status})`, "WEB_PROVIDER_ERROR");
    }
    try {
...
var DuckDuckGoEngine = class {
  ...
  async fetchSerp(query, signal) {
    ...
    if (!response.ok) {
      ...
      throw new WebError6(`DuckDuckGo search request failed (HTTP ${response.status})`, "WEB_PROVIDER_ERROR");
    }
    try {
      return await readCappedText2(response, this.options.maxSerpBytes);
    } catch (error) {
...
var JinaEngine = class {
  ...
  async search(query, maxResults, signal) {
    ...
    if (!response.ok) {
      await response.body?.cancel();
      throw new WebError7(`Jina search request failed (HTTP ${response.status})`, "WEB_PROVIDER_ERROR");
    }
    let body;
    try {
    ...
    try {
      ...
      throw new WebError7(`Jina search returned a non-JSON response: ${String(error)}`, "WEB_PROVIDER_ERROR", { cause: error });
    }
    const sources = [];
...
async function enrichSources(query, sources, keep, options, signal) {
  ...
  if (signal.aborted) throw new WebError8("web search aborted", "WEB_ABORTED");
  ...
  const ranked = candidates.map((source, index) => ({ source, index, score: scores[index] ?? 0 })).sort((a, b) => b.score - a.score || a.index - b.index);
...
async function fetchPageText(url, options, signal) {
  ...
  try {
    ...
    try {
      ...
      if (signal.aborted) throw new WebError8("web search aborted", "WEB_ABORTED", { cause: error });
      return void 0;
    }
    if (!response.ok) {
      await response.body?.cancel();
...
var MultiSearchProvider = class {
  ...
  async fallback(ids, query, maxResults, signal) {
    ...
    throw new WebError9(`all search engines failed: ${errors.join("; ")}`, "WEB_PROVIDER_ERROR");
  }
...
function toWebError(error, message) {
  if (error instanceof WebError9) return error;
  return new WebError9(message, "WEB_ABORTED", { cause: error });
}
function errorMessage(error) {
  if (error instanceof WebError9) return error.message;
  return String(error);
}
...
// src/search/index.ts
var DEFAULT_USER_AGENT2 = "deepseek-harness/0.1.1 (+https://github.com/deepseek-ai)";
var WEB_SEARCH_MULTI_SETTINGS_NAMESPACE = settingsNamespace2("web-search-multi");
var Config4 = z4.object({
  engines: z4.array(z4.string()).default(["ddg", "bing", "exa", "deepseek", "jina"]),
...
function apply4(ctx, config) {
  ...
  const store = new WebStore({ path: config.storePath ?? dshHomePath3("web.db") });
  const engines = [
...
function apply5(ctx, config) {
  apply4(ctx, config.search ?? {});
  apply(ctx, config.fetch ?? {});
  apply3(ctx, config.platforms ?? {});
  apply2(ctx, config.history ?? {});
}
export {
  Config5 as Config,
  apply5 as apply,
