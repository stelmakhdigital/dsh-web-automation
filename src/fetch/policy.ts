/**
 * URL validation and content-type classification for the cached HTTP(S) fetch
 * provider — the pure, network-free half. Intentionally parallel to
 * `@deepseek-ai/dsh-web-fetch-http/policy` (each package owns its dependencies);
 * the two MUST evolve together.
 *
 * @module @deepseek-ai/dsh-web-fetch-cached/policy
 */

/* jscpd:ignore-start -- mirrors @deepseek-ai/dsh-web-fetch-http/policy; MUST evolve together */
